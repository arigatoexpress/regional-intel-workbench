...
async def fetch_fullsail_snapshot() -> ProtocolSnapshot:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=timeout) as client:
        pools_response, config_response, stats_response = await asyncio.gather(
            client.get("/api/pools/voting", params={"page": 0, "page_size": 250, "account_address": ""}),
    ...
    for raw_pool in pools_payload.get("pools", []):
        ...
        weekly_volume = parse_metric_or_zero(str(raw_pool.get("weekly_volume_usd") or dynamic_stats.get("volume_usd_7d") or "0"))
        predicted_volume = normalize_fullsail_prediction_usd(raw_pool.get("predicted_volume_usd"), weekly_volume)
        ...
        current_votes = normalize_fullsail_vote_power(raw_pool.get("total_voting_power")) or 0.0
        total_rewards = parse_metric_or_zero(str(raw_pool.get("voting_fees_usd") or "0"))
        vote_share = (current_votes / total_vote_power * 100) if total_vote_power else None
    ...
    key_stats = [
        ...
        KeyStat(label="Voting power", value=f"{format_number_compact(total_vote_power)} veSAIL"),
        KeyStat(label="Avg voting APR", value=f"{avg_voting_apr:.2f}%"),
        KeyStat(label="Fee rewards", value=format_usd_compact(parse_metric_or_zero(str(overview.get("fee_rewards_usd") or "0")))),
        KeyStat(label="Daily volume", value=format_usd_compact(parse_metric_or_zero(str(overview.get("daily_volume_usd") or "0")))),
        KeyStat(label="TVL", value=format_usd_compact(parse_metric_or_zero(str(statistics.get("tvl_usd") or "0")))),
        KeyStat(label="veSAIL supply", value=format_number_compact(ve_sail_supply)),
        KeyStat(label="Passive fee APR", value=f"{float(current_epoch.get('passive_fee_apr') or 0):.2f}%"),
    ]
    ...
    return ProtocolSnapshot(
        ...
        epoch_label=f"Epoch #{current_epoch.get('epoch_count')}" if current_epoch.get("epoch_count") is not None else None,
        countdown=None,
